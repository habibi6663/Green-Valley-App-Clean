...
import { Menu, X, Bell, Search, LayoutDashboard, FileText, User, Calendar, CreditCard, LogOut, ChevronRight, Megaphone, CalendarRange, Images, BookOpen } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex bg-surface overflow-x-hidden">
      ...
      <aside className={`flex flex-col w-72 bg-surface-container-low border-r border-outline-variant/5 fixed h-screen z-50 transition-transform duration-300 ease-in-out ${
        ...
        <div className="mt-auto p-4 md:p-8 border-t border-outline-variant/5 space-y-4">
          <button
      ...
      <div className="flex-1 flex flex-col lg:pl-72">
        {/* Top Bar */}
        <header className="h-20 flex items-center justify-between px-4 md:px-8 bg-surface/80 backdrop-blur-md sticky top-0 z-30 border-b border-outline-variant/5">
        ...
        {/* Main Content */}
        <main className="flex-1 flex flex-col p-4 md:p-8 max-w-full px-4 md:px-6 mx-auto w-full pb-32 lg:pb-8">
          {children}
        </main>
        ...
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md z-50 flex justify-around items-center p-2 bg-surface-container-high/90 backdrop-blur-xl rounded-2xl border border-outline-variant/10 shadow-2xl lg:hidden">
          {navItems.map((item) => (
            ...
            >
              <item.icon size={18} />
              <span className="hidden md:inline text-[9px] font-bold uppercase tracking-tighter mt-1">{item.label}</span>
            </button>
          ))}
          <button 
            onClick={() => setActiveTab('profile')}
          ...
          >
            <User size={18} />
            <span className="hidden md:inline text-[9px] font-bold uppercase tracking-tighter mt-1">Profile</span>
          </button>
        </nav>
      </div>
    </div>
