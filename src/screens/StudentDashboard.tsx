...
export default function StudentDashboard({ onViewBilling, onViewAssignments, onViewNotes }: StudentDashboardProps) {
  ...
  const [userRollNumber, setUserRollNumber] = React.useState<string>('');
  ...
  React.useEffect(() => {
    ...
    const userUnsubscribe = onSnapshot(doc(db, 'users', uid), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setUserRollNumber(getDisplayRollNumber(data));
    ...
    <div className="space-y-16">
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
        <div className="space-y-2">
          ...
          <p className="text-outline/70 text-xs font-bold tracking-widest uppercase mt-1">
            Roll No: <span className="text-white">{userRollNumber || 'Not assigned'}</span>
          </p>
        </div>
